...
CREATE TABLE IF NOT EXISTS `rede`(
	...
	`vel_download` VARCHAR(100),
	`vel_upload` VARCHAR(100),
    `ping` VARCHAR(100),
    `fkInterfaces` INT ,
  FOREIGN KEY (`fkInterfaces`) REFERENCES `rede`(`id_rede`) ON DELETE CASCADE
);
...
select registro.data_registro,vel_download,vel_upload,ping from registro, rede;
